 ...
 * @author radames
 */
class GUI_Motos extends JFrame {
    ...
    private JTextField tfIDMoto = new JTextField(5);
    private JTextField tfMarca = new JTextField(2);
    private JTextField tfModelo = new JTextField(2);
    ...
    private Controle_Motos controle = new Controle_Motos(); //essa é a classe de processamento (controle da lista de contatos)
    private Contato_Motos contato = new Contato_Motos(); //ver classe contato
    ...
    private void travarTextFields(boolean campo) {
        tfIDMoto.setEditable(campo); //permite que o usuario digite nesse textField
        ...
        public boolean dispatchKeyEvent(KeyEvent e) {
            if (e.getID() == KeyEvent.KEY_PRESSED) {
                if (e.getKeyCode() == KeyEvent.VK_F1) {
                    color = corzinhabg.showDialog(null,"Escolha uma cor para o fundo", Color.ORANGE);
   ...
   return "<html><p style='background:#555;color:yellow'>" + message + "</p></html>"; 
    ...
    public GUI_Motos() {
        //abrir o arquivo
        List<String> listaAuxiliar = fer.abrirArquivo("Catalogo de motos.txt");
        if (listaAuxiliar != null) {
            for (int i = 0; i < listaAuxiliar.size(); i++) {
                String aux[] = listaAuxiliar.get(i).split(";");
                Contato_Motos c = new Contato_Motos(Integer.valueOf(aux[0]), aux[1], aux[2], aux[3]);
                controle.inserir(c);
        ...
        pnCentro.setBackground(Color.ORANGE);
        ...
        pnNorte.add(tfIDMoto);
        tfIDMoto.setToolTipText(getDefaultToolTip("Apenas um textfield"));
...
//        usarGridBagLayoutCentro.add(lbMarca, tfMarca, lbModelo, tfModelo);
        ...
        btGravar.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                fer.salvarArquivo("Catalogo de motos.txt", listaStr);
...
// ------------------------BOTAO BUSCAR ----------------------------------------        
        btBuscar.addActionListener((ActionEvent e) -> {
            ...
            if (tfIDMoto.getText().trim().equals("")) {
                JOptionPane.showMessageDialog(cp, "Querido usuário, vazio não pode (espaços também não)");
            } else {
                try {
                    tfIDMoto.setBackground(Color.green);
                    travarTextFields(true);
                    cardLayout.show(pnSul, "pnMsg");
                    contato = controle.buscar(Integer.valueOf(tfIDMoto.getText()));
                    if (contato == null) { //nao achou
                        btInserir.setVisible(true);
                        btAlterar.setVisible(false);
                        btExcluir.setVisible(false);
                        limparValoresDosAtributos();
                        JOptionPane.showMessageDialog(cp, "A moto(ID) inserida não está disponível no estoque, caso queira adicioná-lo clique em inserir");
                    } else { //achou
                        btAlterar.setVisible(true);
                        btExcluir.setVisible(true);
                        btInserir.setVisible(false);
                        tfIDMoto.setText(String.valueOf(contato.getNomeDoCarro()));
                    ...
                    tfIDMoto.selectAll();
                    tfIDMoto.requestFocus();
                    tfIDMoto.setBackground(Color.red);
...
//*********************** BOTÃO SALVAR ****************************************        
        btSalvar.addActionListener((ActionEvent e) -> {
            Contato_Motos contatoOriginal = contato; //para pesquisar na lista
            //precisamos do contato original (para depois modificar)
            if (inserindo) {
                contato = new Contato_Motos(); //criar um novo contato
            }
            //transfere os valores da GUI para classe contato
            contato.setNomeDoCarro(Integer.valueOf(tfIDMoto.getText()));
        ...
        btExcluir.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                int dialogResult = JOptionPane.showConfirmDialog(cp, "Deseja excluir [ "
                        + tfIDMoto.getText() + "-" + tfMarca.getText() + "]?", "Deletar elemento", NORMAL);
...
//$$$$$$$$$$$$$$ FIM DOS LISTENERS $$$$$$$$$$$$$
        ...
        setSize(840, 240);
        setTitle("Locadora de automóveis (Motos)");
